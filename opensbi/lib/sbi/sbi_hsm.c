...
({									\
	...
	if (state != (oldstate))					\
		sbi_printf("%s: ERR: The hart is in invalid state [%lu], &state=%p\n", \
			   __func__, state, &(hdata)->state);			\
	state == (oldstate);						\
})
					  ...
					  u32 hartid)
{
	...
	/* Debug: print addresses and plain-load value BEFORE atomic_cmpxchg */
	sbi_printf("hart_start_finish: scratch=%p, hart_data_offset=%lu, hdata=%p\n",
		scratch, hart_data_offset, hdata);
	sbi_printf("hart_start_finish: &hdata->state=%p, plain_read=%ld\n",
		&hdata->state, atomic_read(&hdata->state));
...
int sbi_hsm_init(struct sbi_scratch *scratch, bool cold_boot)
{
	...
	if (cold_boot) {
		...
		sbi_for_each_hartindex(i) {
			...
			sbi_printf("sbi_hsm_init: i=%d, current_hartindex()=%ld, hdata->state=%ld, &state=%p\n",
				i, (long)current_hartindex(), (long)atomic_read(&hdata->state), &hdata->state);
			hdata->suspend_type = 0;
