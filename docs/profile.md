...
工具目录 `npc/tools/profiler/`（详见 [npc/tools/profiler/README.md](../npc/tools/profiler/README.md)）。固定 benchmark：**dhrystone**、**coremark**；采集时 `DIFFTEST=` 禁用协同仿真。
...
每次采集写入 **`npc/build/profile/<run_id>/`** 子目录。省略 `PROFILE_OUT_DIR` 时自动使用 `npc/build/profile/<timestamp>/`。

| 路径 | 说明 |
|------|------|
| `<run_id>/dhrystone.json`、`<run_id>/coremark.json` | 单 benchmark JSON |
...
仿真失败时保留 `<run_id>/dhrystone.sim.log`、`<run_id>/coremark.sim.log`。
