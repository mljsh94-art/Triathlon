...
void ProfileCollector::emit_summary_json(uint64_t final_cycles, const Vtb_triathlon *top) {
  ...
  os << "\"flush\":{";
  os << "\"count\":" << flush_count_ << ",\"bru_count\":" << bru_count_
     << ",\"per_kinst\":" << safe_div(static_cast<double>(flush_count_) * 1000.0, static_cast<double>(commits))
  ...
  os << "\"distance_sum\":" << redirect_distance_sum_
     ...
     << ",\"distance_avg\":" << safe_div(static_cast<double>(redirect_distance_sum_),
                                        static_cast<double>(redirect_distance_samples_))
     << ",\"distance_max\":" << redirect_distance_max_;
  os << "},";
  os << "\"reason_histogram\":";
