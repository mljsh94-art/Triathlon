...
/*
  Instruction Fetch Unit (decoupled from BPU via FTQ)
  1. 从 FTQ 读取预测条目驱动 ICache 请求
  2. 用 request FIFO 将 FTQ 消费与 ICache 请求解耦
...
module ifu #(
    ...
    //--- 1.FTQ 读取接口 (从 FTQ 获取预测条目) ---
    input  logic                       ftq_deq_valid_i,    // FTQ 有可用条目
    output logic                       ftq_deq_ready_o,    // IFU 可以消费
    input  logic [Cfg.PLEN-1:0]        ftq_deq_pc_i,
    input  logic                       ftq_deq_pred_slot_valid_i,
    input  logic [SLOT_IDX_W-1:0]      ftq_deq_pred_slot_idx_i,
    input  logic [Cfg.PLEN-1:0]        ftq_deq_pred_target_i,
    input  logic [Cfg.PLEN-1:0]        ftq_deq_pred_npc_i,
    input  logic [EPOCH_W-1:0]         ftq_deq_epoch_i,
    input  logic [FTQ_ID_W-1:0]        ftq_deq_ftq_id_i,
    ...
    output logic [Cfg.INSTR_PER_FETCH-1:0][FTQ_ID_W-1:0] ifu_ibuffer_rsp_ftq_id_o, // 指令对应分配的 FTQ ID
  ...
  // Pending request FIFO (from FTQ dequeue).
  ...
  // FTQ dequeue consumption control
  logic ftq_deq_fire_w;
...
`endif
  ...
  assign local_mmu_replay_pc_w =
      ...
      (!req_fifo_empty_w ? req_head_pc_w : '0);

  // === FTQ dequeue consumption ===
  // IFU consumes from FTQ when: not flushing, no fault pending, req_fifo has space
  assign ftq_deq_ready_o = !ifu_flush_w && !fault_pending_q && !fault_wait_flush_q &&
                           (!req_fifo_full_w || req_pop_w);
  assign ftq_deq_fire_w = ftq_deq_valid_i && ftq_deq_ready_o;
...
`endif
  ...
  // NOTE: FTQ instance removed from IFU — now at frontend top level
...
`endif
      ...
      if (flush_i) begin
        ...
        // On flush, FTQ deq may fire in the same cycle if ftq_deq_ready was already asserted.
        // We accept this entry as the first request in the new epoch.
        req_head_q <= '0;
        if (ftq_deq_fire_w) begin
          req_pc_fifo_q['0] <= ftq_deq_pc_i;
          req_pred_slot_valid_fifo_q['0] <= ftq_deq_pred_slot_valid_i;
          req_pred_slot_idx_fifo_q['0] <= ftq_deq_pred_slot_idx_i;
          req_pred_target_fifo_q['0] <= ftq_deq_pred_target_i;
          req_ftq_id_fifo_q['0] <= ftq_deq_ftq_id_i;
        ...
        // Normal operation: consume FTQ dequeue entries into req_fifo
        if (ftq_deq_fire_w) begin
        ...
        unique case ({ftq_deq_fire_w, req_pop_w})
...
`ifdef TRIATHLON_VERBOSE
    $display("ifu_ftq(deq_v/deq_r/fire): %0d/%0d/%0d", ftq_deq_valid_i,
             ftq_deq_ready_o, ftq_deq_fire_w);
...
`ifndef SYNTHESIS
    ...
    if (ifu_diag_trace_en_q && (flush_i || local_mmu_flush_w) &&
        ...
        (((redirect_pc_i & 32'hf0000000) == 32'hc0000000) ||
         ((req_head_pc_w & 32'hf0000000) == 32'hc0000000))) begin
      $display("[ifu-flush] flush_i=%0d local=%0d satp_changed=%0d sfence=%0d redir=%h req_cnt=%0d inf_cnt=%0d ftq_deq=%0d epoch=%0d->%0d",
               flush_i, local_mmu_flush_w, satp_changed_w, mmu_sfence_vma_i, redirect_pc_i,
               req_count_q, inf_count_q, ftq_deq_fire_w, fetch_epoch_q, flush_next_epoch_w);
      ifu_pc_dbg_cnt_q <= ifu_pc_dbg_cnt_q + 32'd1;
    end
